...
function resolveForMode(varId, modeId, variablesById, idToPath, seen = new Set()) {
    ...
    let raw = (_a = v.valuesByMode) === null || _a === void 0 ? void 0 : _a[modeId];
    ...
    if (raw.type === 'VARIABLE_ALIAS') {
        ...
        d('RESOLVE/alias', { from: v.name, toId: raw.id, toPath: (_b = path === null || path === void 0 ? void 0 : path.join('.')) !== null && _b !== void 0 ? _b : '(unknown)' });
...
async function getAllVariables() {
    ...
    try {
        ...
        for (const v of variables)
            idToPath.set(v.id, v.name.split('/').map(s => s.trim()).filter(Boolean));
        ...
        const panelItems = variableCollections.map((c) => {
            ...
            const modes = c.modes.map(m => {
                ...
                const count = vars.reduce((n, v) => { var _a; return n + (((_a = v.valuesByMode) === null || _a === void 0 ? void 0 : _a[m.modeId]) ? 1 : 0); }, 0);
                return {
                    modeId: m.modeId,
                    modeName: m.name,
                    count,
        ...
        for (const coll of variableCollections) {
            ...
            for (const mode of coll.modes) {
                ...
                for (const v of varsInColl) {
                    ...
                    if ((_b = v.description) === null || _b === void 0 ? void 0 : _b.trim())
        ...
        for (const style of paintStyles) {
            ...
            if (style.paints && style.paints.length > 0) {
                ...
                if (paint.type === 'SOLID') {
                    const { r, g, b } = paint.color;
                    const a = (_c = paint.opacity) !== null && _c !== void 0 ? _c : 1;
        ...
        Object.entries(categorizedVariables).forEach(([collectionModeKey, nestedData]) => {
            ...
            function flattenNested(obj, path = []) {
        ...
        console.log('Total variables across all collections:', Object.values(uiVariables).reduce((sum, vars) => sum + (Array.isArray(vars) ? vars.length : 0), 0));
...
function transformToTokenStudio(tokens) {
    ...
    Object.entries(groupedCollections).forEach(([baseCollectionName, modes]) => {
        ...
        Object.entries(modes).forEach(([modeName, variables]) => {
            console.log(`Processing mode "${modeName}" in collection "${baseCollectionName}" with ${variables.length} variables`);
