...
export function transformToStyleDictionary(tokens: any): any {
  const sdTokens: any = {};
...
export async function generateStyleDictionaryOutput(tokens: any, platform: string): Promise<string> {
...
function getTransformGroup(platform: string): string {
  const transformGroups: { [key: string]: string } = {
    'css': 'css',
    'scss': 'scss',
    'js': 'js',
    'ts': 'js',
